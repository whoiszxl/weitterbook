...
```bash
# 创建MySQL网段
docker network create --subnet 172.18.0.0/18 wts-net

# MySQL-001
docker run --name mysql-001 \
--net wts-net --ip 172.18.0.2 \
-m 400m \
...
dataSources:
 rep_s1_member:
   url: jdbc:mysql://172.18.0.2:3306/wts-member?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&nullCatalogMeansCurrent=true&allowPublicKeyRetrieval=true
   username: root
   password: 123456
   ...
   minPoolSize: 1
 rep_s2_member:
   url: jdbc:mysql://172.18.0.3:3306/wts-member?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&nullCatalogMeansCurrent=true&allowPublicKeyRetrieval=true
   username: root
   password: 123456
   connectionTimeoutMilliseconds: 30000
   idleTimeoutMilliseconds: 60000
   ...
   minPoolSize: 1
 rep_s3_member:
   url: jdbc:mysql://172.18.0.4:3306/wts-member?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&nullCatalogMeansCurrent=true&allowPublicKeyRetrieval=true
   username: root
   password: 123456
   connectionTimeoutMilliseconds: 30000
   idleTimeoutMilliseconds: 60000
 ...
 tables:
   ums_member:
     ...
     databaseStrategy:
       standard:
         shardingColumn: id
         shardingAlgorithmName: wts-inline
     keyGenerateStrategy:
       column: id
   ...
   ums_address:
     ...
     databaseStrategy:
       standard:
         ...
         shardingAlgorithmName: wts-children-inline
     keyGenerateStrategy:
       column: id
       keyGeneratorName: snowflake
...
创建容器
```bash
docker run -v /opt/data/docker/redis/conf:/usr/local/etc/redis -p6379:6379 --net wts-net --ip 172.18.0.7 --name wts-redis -d redis
```
